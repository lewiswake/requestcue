...
export async function initGuestMode(targetDjId) {
  // Elements
  ...
  const aboutPhotoDisplay = document.getElementById("aboutPhotoDisplay");
  ...
  try {
    if (targetDjId.startsWith("uid:")) {
      djId = targetDjId.slice(4);
      const userSnap = await getDoc(doc(db, "users", djId));
      if (userSnap.exists()) {
        d = userSnap.data();
      }
    } else {
      const q = query(collection(db, "users"), where("username", "==", targetDjId));
    ...
    if (d && djId) {
      ...
      // About
      if (d.aboutText || d.photoURL) {
        if (d.aboutText) {
          document.getElementById("aboutTextDisplay").textContent = d.aboutText;
        }
        if (d.photoURL) {
          aboutPhotoDisplay.src = d.photoURL;
          aboutPhotoDisplay.hidden = false;
        }
